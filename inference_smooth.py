...
import torch
from inference_utils import load_model, predict_masks, draw_smooth_contours

# ---------------- CONFIG ----------------
CHECKPOINT_PATH = "model_limbus_crop_unetpp_weighted.pth"
INPUT_DIR = r"C:\Users\satyam.tripathi\Downloads\Suture_Model\test"
OUT_DIR = r"C:\Users\satyam.tripathi\Downloads\Suture_Model\test\output_smooth"
# ----------------------------------------
...
def main():
    ...
    print(f"Smooth Inference | Device: {device} | Images: {len(paths)}")
    ...
    for p in tqdm(paths):
        ...
        # Draw Smooth
        vis = draw_smooth_contours(bgr, masks[idx_crop], masks[idx_limbus])
        
        # Save
        base = os.path.splitext(os.path.basename(p))[0]
        cv2.imwrite(os.path.join(OUT_DIR, f"{base}_smooth.jpg"), vis)
